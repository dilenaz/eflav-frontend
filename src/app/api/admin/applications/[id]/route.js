...
import { updateApplicationStatus } from '@/lib/services/admin-records.service';
...
export async function PATCH(request, { params }) {
  try {
    ...
    const updated = await updateApplicationStatus(Number(id), validation.data.status);
    if (!updated) {
      return NextResponse.json({ success: false, message: 'Başvuru bulunamadı.' }, { status: 404 });
    }
    await writeAuditLog({adminId:Number(admin.sub),action:'application.status_changed',entityType:'application',entityId:id,metadata:{status:validation.data.status}});
    return NextResponse.json({ success: true, message: 'Başvuru güncellendi.' });
  } catch (error) {
    console.error('Başvuru güncelleme hatası:', error);
    return NextResponse.json({ success: false, message: 'Başvuru güncellenemedi.' }, { status: 500 });
  }
}
